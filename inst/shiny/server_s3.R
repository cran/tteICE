...
hsn_32_plot1 <- eventReactive(input$B_32_surv,{
  if(length(hsn_32())!=0) plot(hsn_32(), type="ate", decrease = as.logical(input$d_320), conf.int = input$conf, ylim=input$yrange)
})
hsn_32_plot2 <- eventReactive(input$B_32_surv,{
  if(length(hsn_32())!=0) {
    plot(hsn_32(), type="inc", decrease = as.logical(input$d_320), conf.int = input$conf, ylim=input$yrangecif, 
